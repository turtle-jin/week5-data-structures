...
bool check(char* word)
{
    ...
    node *cursor = root;  //set a node pointer to the root
    int length = strlen(word);  // get the string length of the input word
    for (int i = 0; i < length; i++)
    {
        int index = tolower(word[i]) - 'a';
        if (cursor->children[index] == NULL)  //if children[index] is NULL, then the word does not exist
        {
            return false;
        }
        else
        {
            cursor = cursor->children[index];  //move the pointer to the node if found
        }
    }
